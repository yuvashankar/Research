...
int main(int argc, char const *argv[])
{
    ...
    readFlag = edfread_digital_samples(handle, numberOfChannels - 2, samplesToRead, rawStatus);
    assert (readFlag != -1);
    ...
    seekStartTime = triggerTime - PRE_EVENT_TIME;
    ...
    printf("\nread %i samples, started at %f seconds from start of file: and ended at %f, with duration: %f\n\n",
        samplesToRead, seekStartTime, seekEndTime, seekDuration);
    printf("\n\n");
