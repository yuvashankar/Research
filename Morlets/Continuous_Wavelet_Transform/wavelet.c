	...
	double* result)
{
	...
	const int start = (int) floor( log2( 1.0/(s0 * maximum_frequency * FOURIER_WAVELENGTH_FACTOR) ) /dj);
	...
	#pragma omp parallel private(i, j) shared (result, period, dj, s0, sampling_frequency, J, n, fft_data) default(none)
	{
		double value;
